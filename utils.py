...
def postprocess(images, min_val=-1.0, max_val=1.0):
    ...
    """
    images = np.array([images.detach().cpu().numpy()])
...
def load_generator(model_name):
    ...
    checkpoint_path = os.path.join(CHECKPOINT_DIR, model_name + '_generator.pth')
    ...
    #else:
    model_specific_vars = ['truncation.truncation']
    for var_name in model_specific_vars:
      state_dict[var_name] = generator.net.state_dict()[var_name]
    generator.net.load_state_dict(state_dict)
    #generator = generator.net.cuda()
    generator.net.eval()
...
def factorize_weight(generator, layer_idx='all'):
    ...
    for idx in layers:
        ...
        if gan_type == 'pggan':
            ...
            weight = generator.net.synthesis.__getattr__(layer_name).epilogue.style_mod.dense.fc.weight.T
